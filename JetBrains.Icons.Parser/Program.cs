...
namespace JetBrains.Icons.Parser
{
    ...
    public class JetBrainsIconDataProviderGenerator
    {
        public List<SvgIcon> ParseSvg(string filePath)
        {
            ...
            try
            {
                ...
                {
                    string d;
                    if (path.Name.LocalName == "polygon")
                    {
                        var points = path.Attribute("points")?.Value;
                        d = points != null ? "M" + points : null;
                    }
                    else
                    {
                        d = path.Attribute("d")?.Value;
                    }
        ...
        public void GenerateIconKindEnum(string outputPath, List<string> iconNames)
        {
        ...
        public void ProcessSvgFiles(string directoryPath, string outputProviderPath, string outputEnumPath)
        {
            var svgFiles = Directory.GetFiles(directoryPath, "*.svg", SearchOption.AllDirectories);
            ...
            GenerateCode(outputProviderPath, iconsData);
            GenerateIconKindEnum(outputEnumPath, iconNames);
        }
    }
    ...
    class Program
    {
        ...
        {
            var provider = new JetBrainsIconDataProviderGenerator();
            ...
            string outputDirectory = Path.Combine(projectDirectory, "..", "JetBrains.Icons");
            ...
            string outputProviderPath = Path.Combine(outputDirectory, "JetBrainsIconDataProviderPart.cs");
            string outputEnumPath = Path.Combine(outputDirectory, "JetBrainsIconKind.cs");

            if (Directory.Exists(iconsDirectory))
            {
                provider.ProcessSvgFiles(iconsDirectory, outputProviderPath, outputEnumPath);
            }
            else
            {
